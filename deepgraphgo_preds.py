...
@ck.option(
    '--dgg-scores-file', '-tsf', default=f'data-netgo/DeepGraphGO-Ensemble-{ont}-test.txt',
    help='DeepGraphGO predictions')
@ck.option(
    '--out_file', '-of', default=f'data-netgo/{ont}/predictions_dgg.pkl', help='Output file')
def main(test_data_file, terms_file,
         dgg_scores_file, out_file):
    ...
    dgg_scores = {}
    with open(dgg_scores_file) as f:
        for line in f:
            it = line.strip().split()
            p_id, go_id, score = it[0], it[1], float(it[2])
            if p_id not in dgg_scores:
                dgg_scores[p_id] = {}
            dgg_scores[p_id][go_id] = score
    preds = []
    print('DGG preds')
    ...
    for i, row in enumerate(test_df.itertuples()):
        ...
        if prot_id in dgg_scores:
            annots = dgg_scores[prot_id]
